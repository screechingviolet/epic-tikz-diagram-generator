...
def tikz_to_geo_programmatic(tikz: str) -> list[str]:
    """
    Parse TikZ code into geometry primitives using regex.
    Works on TikZ generated by MODE1_SYSTEM which follows a known template.
    """
    primitives = []
    point_coords = {}  # name -> (x, y) for building lines

    # match: \filldraw (x, y) circle (2pt) node[...] {$NAME$};
    # also handles: \filldraw[...] (x, y) circle (2pt) node[...] {$NAME$}
    point_pat = re.compile(
        r'\\filldraw[^(]*\(\s*([+-]?\d+\.?\d*)\s*,\s*([+-]?\d+\.?\d*)\s*\)'
        r'.*?node\s*\[[^\]]*\]\s*\{\s*\$?\s*([^$}]+?)\s*\$?\s*\}',
        re.DOTALL
    )
    ...
    # match: \draw (x1,y1) -- (x2,y2) node[midway,...] {$NAME$}
    line_pat = re.compile(
        r'\\draw[^(]*\(\s*([+-]?\d+\.?\d*)\s*,\s*([+-]?\d+\.?\d*)\s*\)'
        r'\s*--\s*'
        r'\(\s*([+-]?\d+\.?\d*)\s*,\s*([+-]?\d+\.?\d*)\s*\)'
        r'.*?node\s*\[[^\]]*\]\s*\{\s*\$?\s*([^$}]+?)\s*\$?\s*\}',
        re.DOTALL
    ...
    for m in line_pat.finditer(tikz):
        ...
        p2_name = _find_point_by_coord(point_coords, x2, y2)

        if p1_name and p2_name:
            primitives.append(f"line({name}, {p1_name}, {p2_name})")

    # match: \draw (cx,cy) circle (r) node[above=...] {$NAME$}
    circle_pat = re.compile(
        r'\\draw[^(]*\(\s*([+-]?\d+\.?\d*)\s*,\s*([+-]?\d+\.?\d*)\s*\)'
        r'\s*circle\s*\(\s*([+-]?\d+\.?\d*)\s*\)'
        r'.*?node\s*\[[^\]]*\]\s*\{\s*\$?\s*([^$}]+?)\s*\$?\s*\}',
        re.DOTALL
    )
    for m in circle_pat.finditer(tikz):
        ...
        if center_name:
            primitives.append(f"circle({name}, {center_name}, {r:.4f})")

    return primitives


def _find_point_by_coord(point_coords: dict, x: float, y: float, tol: float = 1e-3) -> str | None:
    """Find a point name by its coordinates within tolerance."""
    for name, (px, py) in point_coords.items():
    ...
    if to_query:
        ...
        for j, (i, nl) in enumerate(zip(indices, to_query)):
            ...
            record = {
                ...
                "pred_geo": pred_geo,
                "truth_constraints": truth_constraints_list[i],
                "score": score,
            }
...
def run_benchmark():
    ...
    scores = {
        model: {
            "mode1": {"compiles": 0, "score_sum": 0.0, "total": 0},  # add score_sum
    ...
    for model_name, provider in MODELS.items():
        ...
        if 1 in modes:
            ...
            for b, idx_batch in enumerate(batches):
                ...
                constrs_batch = [constrs[i] for i in idx_batch]   # pass constraints
                print(f"    Batch {b+1}/{len(batches)} ...", end=" ", flush=True)
                r1s = run_mode1_batch(clients, nls_batch, constrs_batch, model_name, provider, cache)
                n_ok = sum(r["compiles"] for r in r1s)
                avg  = sum(to_binary(r.get("score", 0)) for r in r1s) / len(r1s)
                print(f"{n_ok}/{len(r1s)} compiled  constraint score {avg:.2f}")
                for r in r1s:
                    all_results.append(r)
                    scores[model_name]["mode1"]["total"]     += 1
                    scores[model_name]["mode1"]["compiles"]  += int(r["compiles"])
                    scores[model_name]["mode1"]["score_sum"] += to_binary(r.get("score", 0))
                    if r["compiles"]:
                        save_compiled_tex(r)
    ...
    for model_name in MODELS:
        ...
        summary[model_name] = {
            ...
            "mode1_constraint_score":  m1["score_sum"] / max(m1["total"], 1),  # new
    ...
    print("\n" + "=" * 65)
    print(f"{'Model':<35} {'NL->TikZ':>10} {'TikZ->Geo':>10} {'NL->Geo':>10} {'GT->Geo':>10}")
    print("=" * 75)
    for model_name, s in summary.items():
        m1c = f"{s['mode1_compile_rate']:.1%}"      if scores[model_name]["mode1"]["total"] > 0 else "—"
        m1s = f"{s['mode1_constraint_score']:.2f}"  if scores[model_name]["mode1"]["total"] > 0 else "—"
        ...
        print(f"{model_name:<35} {m1c:>10} {m1s:>10} {m2:>10} {m3:>10}")
    print("=" * 65)
